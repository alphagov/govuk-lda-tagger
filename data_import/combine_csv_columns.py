...
if __name__ == '__main__':
    ...
    writer = csv.DictWriter(sys.stdout, ('url', 'text'))
    wordy_columns = set('title,description,content,topics,organisations,pdfdata'.split(','))
    writer.writeheader()

    for row in reader:
        text = ' '.join([format_value(value) for key, value in row.iteritems() if key in wordy_columns])
        writer.writerow(dict(url=row['url'], text=text))
